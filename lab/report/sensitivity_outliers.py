...
def fmt0(v):
    """Format a number with thousands separators, or '--' when absent.
    A configuration can be present in only one of the two runs (a newly added
    implementation has no run-1 counterpart)."""
...
def main():
    ...
    for config in sorted(set(g1) | set(g2)):
        ...
        m1 = stats.mean([r[1] for r in a])
        m2 = stats.mean([r[1] for r in b])
        med2 = statistics.median([r[1] for r in good2]) if good2 else None
        w(f"| {config} | {fmt0(m1)} | {len(slow1)} | "
          f"{fmt0(m2)} | {len(slow2)} | "
          f"{fmt0(med2)} |")
    w("")
    ...
    w("- The `etcd-core` configuration is the same etcd/raft core with")
    w("  in-memory storage only, so it has no fsync path at all; it is listed")
    w("  here for completeness and is excluded from the fsync interpretation.")
