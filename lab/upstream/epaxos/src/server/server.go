...
var commCostMS = flag.Int("comm-cost-ms", 0, "Fixed one-way latency (ms) added to every inter-replica message; 0 = local-network baseline.")
var commJitter = flag.Int("comm-jitter-pct", 0, "Jitter as % of comm-cost-ms: each message waits an extra uniform delay in [0, cost*jitter/100].")
...
func main() {
	...
	if *doEpaxos {
		...
		rep.CommCostMS = *commCostMS
		rep.CommJitterPct = *commJitter
		rpc.Register(rep)
	} else if *doMencius {
